...
function parseStop(data){
  ...
  return visits.map(v=>{
    ...
    return { lineId, lineCode, lineLabel, direction, dest: destDisplay || direction || "—", minutes: minutesFromISO(expected), status, rawLineRef: lineRef };
  });
}
...
async function renderLineBoard(container, visits, options={}){
  ...
  visits.forEach(visit=>{
    const key=((visit.lineCode||visit.lineLabel||visit.lineId||visit.rawLineRef||"?").toString().trim())||"?";
